...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {aj: fragment, an: host, as: path, au: port_, ax: protocol, ay: query};
	});
...
var $author$project$Student$NamePage = {$: 0};
...
var $author$project$Student$init = function (_v0) {
	return _Utils_Tuple2(
		{a: '', t: _List_Nil, g: $elm$core$Maybe$Nothing, j: '', n: '', q: '', i: $author$project$Student$NamePage, F: '', ab: $elm$core$Maybe$Nothing},
		$author$project$Student$requestBelts(0));
};
...
var $author$project$Student$Belt = F5(
	function (id, name, color, order, gameOptions) {
		return {R: color, ak: gameOptions, h: id, p: name, W: order};
	});
...
var $author$project$Student$Student = F5(
	function (id, name, created, lastActive, submissions) {
		return {af: created, h: id, ap: lastActive, p: name, Q: submissions};
	});
var $author$project$Student$Submission = F8(
	function (id, studentId, beltLevel, gameName, githubLink, notes, submissionDate, grade) {
		return {a: beltLevel, j: gameName, n: githubLink, T: grade, h: id, q: notes, aa: studentId, P: submissionDate};
	});
var $author$project$Student$Grade = F4(
	function (score, feedback, gradedBy, gradingDate) {
		return {a_: feedback, a0: gradedBy, a1: gradingDate, K: score};
	});
	...
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'id',
				$elm$json$Json$Encode$string(submission.h)),
				_Utils_Tuple2(
				'studentId',
				$elm$json$Json$Encode$string(submission.aa)),
				_Utils_Tuple2(
				'beltLevel',
				$elm$json$Json$Encode$string(submission.a)),
				_Utils_Tuple2(
				'gameName',
				$elm$json$Json$Encode$string(submission.j)),
				_Utils_Tuple2(
				'githubLink',
				$elm$json$Json$Encode$string(submission.n)),
				_Utils_Tuple2(
				'notes',
				$elm$json$Json$Encode$string(submission.q)),
				_Utils_Tuple2(
				'submissionDate',
				$elm$json$Json$Encode$string(submission.P))
			]));
};
...
var $author$project$Student$isValidNameFormat = function (name) {
...
var $author$project$Student$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 1:
				var trimmedName = $elm$core$String$trim(model.F);
				return $elm$core$String$isEmpty(trimmedName) ? _Utils_Tuple2(
					_Utils_update(
						model,
						{
							g: $elm$core$Maybe$Just('Please enter your name to continue')
						}),
					$elm$core$Platform$Cmd$none) : ((!$author$project$Student$isValidNameFormat(trimmedName)) ? _Utils_Tuple2(
					_Utils_update(
						model,
						{
							g: $elm$core$Maybe$Just('Please enter your name in the format firstname.lastname (e.g., tyler.smith)')
						}),
					...
					_Utils_update(
						model,
						{
							...
							i: $author$project$Student$LoadingPage('Searching for your record...')
						}),
					$author$project$Student$findStudent(trimmedName)));
			case 2:
				var result = msg.a;
				if (!result.$) {
					...
					if (!maybeStudent.$) {
						...
						return _Utils_Tuple2(
							_Utils_update(
								...
								{
									g: $elm$core$Maybe$Nothing,
									i: $author$project$Student$StudentProfilePage(student)
								}),
							$elm$core$Platform$Cmd$none);
					} else {
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									g: $elm$core$Maybe$Just('No record found. Please check your name or ask your teacher to create a record for you.'),
									i: $author$project$Student$NamePage
								}),
							$elm$core$Platform$Cmd$none);
					}
				} else {
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							...
							n: '',
							q: '',
							i: $author$project$Student$SubmissionFormPage(student)
						}),
					$elm$core$Platform$Cmd$none);
			case 4:
				var beltId = msg.a;
				var gameOptions = A2($author$project$Student$getGameOptions, beltId, model.t);
				var defaultGame = A2(
			...
			case 8:
				var student = msg.a;
				if (($elm$core$String$trim(model.a) === '') || (($elm$core$String$trim(model.j) === '') || ($elm$core$String$trim(model.n) === ''))) {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								g: $elm$core$Maybe$Just('Please fill in all required fields')
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					...
					var newSubmission = {
						...
						h: student.h + ('-' + (model.a + ('-' + $elm$core$String$fromInt(
							$elm$core$List$length(student.Q) + 1)))),
						q: model.q,
						aa: student.h,
						P: currentDate
					};
					return _Utils_Tuple2(
						_Utils_update(
							...
							{
								g: $elm$core$Maybe$Nothing,
								i: $author$project$Student$LoadingPage('Saving your submission...')
							}),
			...
			case 9:
				...
				if (_v3.$ === 4) {
					return A2($elm$core$String$startsWith, 'Error:', result) ? _Utils_Tuple2(
						...
						$elm$core$Platform$Cmd$none) : _Utils_Tuple2(
						model,
						$author$project$Student$findStudent(model.F));
				} else {
			...
			case 11:
				return _Utils_Tuple2(
					...
					$elm$core$Platform$Cmd$none);
			case 12:
				return $author$project$Student$init(0);
			default:
				var result = msg.a;
				if (!result.$) {
					var belts = result.a;
...
var $author$project$Student$SearchStudent = {$: 1};
...
var $author$project$Student$BackToSearch = {$: 11};
...
var $author$project$Student$formatDisplayName = function (name) {
	var parts = A2($elm$core$String$split, '.', name);
	var lastName = A2(
		$elm$core$Maybe$withDefault,
		'',
...
var $author$project$Student$viewGradeStatus = function (maybeGrade) {
	if (!maybeGrade.$) {
		...
		var _v1 = (grade.K >= 90) ? _Utils_Tuple2('bg-green-100', 'text-green-800') : ((grade.K >= 70) ? _Utils_Tuple2('bg-blue-100', 'text-blue-800') : ((grade.K >= 60) ? _Utils_Tuple2('bg-yellow-100', 'text-yellow-800') : _Utils_Tuple2('bg-red-100', 'text-red-800')));
		var bgColor = _v1.a;
		var textColor = _v1.b;
		return A2(
			$elm$html$Html$span,
			...
			_List_fromArray(
				[
					$elm$html$Html$text(
					$elm$core$String$fromInt(grade.K) + '/100')
				]));
	} else {
		return A2(
			$elm$html$Html$span,
	...
	function (model, submission) {
		var beltName = function () {
			var matchingBelts = A2(
				$elm$core$List$filter,
				function (b) {
					return _Utils_eq(b.h, submission.a);
				},
				model.t);
			if (!matchingBelts.b) {
				return submission.a;
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text(submission.P)
						])),
					A2(
					$elm$html$Html$td,
					_List_fromArray(
					...
					_List_fromArray(
						[
							$author$project$Student$viewGradeStatus(submission.T)
						]))
				]));
	});
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('border-b border-gray-400 pb-5')
						]),
					_List_fromArray(
						[
							A2(
							...
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											$elm$html$Html$text(
											'Welcome, ' + $author$project$Student$formatDisplayName(student.p))
										])),
									A2(
									$elm$html$Html$button,
									_List_fromArray(
					...
					_List_fromArray(
						[
							...
							$elm$core$List$isEmpty(student.Q) ? A2(
							$elm$html$Html$div,
							_List_fromArray(
								[
								...
								[
									...
									_List_fromArray(
										[
											...
											_List_fromArray(
												[
													$elm$html$Html$Attributes$class('divide-y divide-gray-400 divide-opacity-4 bg-white')
												]),
											A2(
												$elm$core$List$map,
...
var $author$project$Student$BackToProfile = {$: 10};
				...
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text('Thank you, ' + (student.p + '! Your Unity game project has been submitted.'))
								]))
						])),
					A2(
					$elm$html$Html$div,
								...
								[
									...
									_List_fromArray(
										[
											...
											_List_fromArray(
												[
													$elm$html$Html$Attributes$href(submission.n),
...
var $author$project$Student$viewBeltColorIndicator = F2(
	function (selectedBeltId, belts) {
		var selectedBelt = $elm$core$List$head(
			A2(
				$elm$core$List$filter,
				function (b) {
					return _Utils_eq(b.h, selectedBeltId);
				},
				belts));
		if (!selectedBelt.$) {
			var belt = selectedBelt.a;
			return A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
...
var $author$project$Student$viewSubmissionFormPage = F2(
	function (model, student) {
		var sortedBelts = A2(
			...
			model.t);
		var gameOptions = A2($author$project$Student$getGameOptions, model.a, model.t);
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text('New Submission for ' + student.p)
						])),
					A2(
					$elm$html$Html$p,
					_List_fromArray(
										...
										[
											...
											_List_fromArray(
												[
													...
													$elm$html$Html$Attributes$class('mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm pl-8')
												]),
											_Utils_ap(
												_List_fromArray(
													[
												...
												A2(
													...
													function (belt) {
														return A2(
															...
															_List_fromArray(
																[
																	$elm$html$Html$Attributes$value(belt.h)
																]),
															_List_fromArray(
																[
																	$elm$html$Html$text(belt.p)
																]));
													},
													sortedBelts))),
											A2($author$project$Student$viewBeltColorIndicator, model.a, sortedBelts)
										]))
								])),
							A2(
							$elm$html$Html$div,
...
var $author$project$Student$view = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('min-h-screen bg-gray-300 py-6 flex flex-col justify-center sm:py-12')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('relative px-4 py-10 bg-gray-100 shadow-lg sm:rounded-lg sm:p-20')
							]),
						_List_fromArray(
							[
								A2(
								...
								_List_fromArray(
									[
										...
										$author$project$Student$viewError(model.g),
										$author$project$Student$viewSuccess(model.ab)
									]))
							]))
					]))
			]));
...
var $author$project$Student$main = $elm$browser$Browser$element(
	{a5: $author$project$Student$init, bf: $author$project$Student$subscriptions, bi: $author$project$Student$update, bj: $author$project$Student$view});
